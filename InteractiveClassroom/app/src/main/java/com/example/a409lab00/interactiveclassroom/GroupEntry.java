...
public class GroupEntry extends AppCompatActivity {
        ...
        LV_GroupList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
                ...
                if (QuestionQueue.size() == 0) {
                    ...
                    ITquestion.putExtra("quinfo",GroupList.get(position).AnswerInfo);
    ...
    List<QGroup> currentQuestions(int cid) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                group.AnswerInfo=que.getString("myAnswer");
                group.total=que.getInt("total");
                group.answeredCount=answeredCount(que.getString("myAnswer"));
                groups.add(group);
            }
    ...
    int answeredCount(String response)
    {
        int cnt =0;
        try{
    ...
    class QGroup
    {
       ...
       public String toString()
        {
            return "["+title+"]"+"[完成度："+answeredCount+"/"+total+"]"+",結束日期："+stop;
        }
    }
}
